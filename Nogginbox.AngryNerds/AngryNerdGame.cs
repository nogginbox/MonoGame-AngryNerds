...
using Nogginbox.AngryNerds.Sprites;
using System;
...
namespace Nogginbox.AngryNerds
{
	public class AngryNerdGame : Game
	{
		...
		private AngryNerdSpriteFactory _nerdFactory;
		...
		protected override void LoadContent()
		{
			...
			_nerdFactory = new AngryNerdSpriteFactory(_world, Content);
			var nerdObjects = new List<SpriteBody>
			{
				_nerdFactory.CreateAngryNerd(new Vector2(10, 10))
			};
			var newNerds = CreateNerdPyramid(450, 5);
			nerdObjects.AddRange(newNerds);
			_nerdObjects = nerdObjects.ToList();
		}

		private IEnumerable<SpriteBody> CreateNerdPyramid(int firstNerdX, int nerdHeight)
		{
			...
			return nerdPositions.Select(nerdPosition => _nerdFactory.CreateAngryNerd(nerdPosition));
		}
		...
		protected override void Update(GameTime gameTime)
		{
			...
			if (position != null)
			{
				...
				else if (_nerdObjects.Count < 100)
				{
					_nerdObjects.Add(_nerdFactory.CreateAngryNerd(new Vector2(position.Value.X, position.Value.Y)));
				}
			...
			{
				_activeNerd.UpdateApplyForce(force.Value * 6);
			}
